...
gulp.task("css-libs", function() {
  return gulp
    .src("css/**/*.css") // Выбираем файл для минификации
...
gulp.task("img", function() {
  return gulp
    .src("images/**/*") // Берем все изображения из app
    .pipe(
      cache(
        imagemin({
          // С кешированием
...
gulp.task("build", ["clean", "img", "css-libs", "scripts"], function() {
  var buildCss = gulp.src("app/css/**/*.css").pipe(gulp.dest("../css"));

  var buildFonts = gulp
    .src("fonts/**/*") // Переносим шрифты в продакшен
    .pipe(gulp.dest("../fonts"));

  var buildpdf = gulp.src("cv/**/*").pipe(gulp.dest("../cv"));

  var buildJs = gulp
    .src("app/js/**/*") // Переносим скрипты в продакшен
    .pipe(gulp.dest("../js"));

  var buildJs = gulp.src("app/images/**/*").pipe(gulp.dest("../images"));

  var buildlibs = gulp
    .src("libs/**/*") // Переносим скрипты в продакшен
    .pipe(gulp.dest("../js"));

  var buildHtml = gulp
    .src("*.html") // Переносим HTML в продакшен
    .pipe(gulp.dest("../"));
});
